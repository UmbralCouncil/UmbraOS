{ lib, pkgs, settings ? null, isLive ? false, ... }:
let
  homeShell = {
    programs.zsh = {
      enable = true;
      ...
      history = { size = 10000; save = 10000; ignoreDups = true; share = true; };
      shellAliases = {
        trash = "gio trash";
        ll = "ls -lah";
    ...
    programs.starship = {
      ...
      settings = {
        ...
        character = { success_symbol = "[❯](bold #9d7cff)"; error_symbol = "[❯](bold red)"; };
        directory.style = "bold #50b7f5";
        git_branch = { symbol = "󰘬 "; style = "bold #9d7cff"; };
        nix_shell = { symbol = " "; style = "bold #50b7f5"; };
      };
    };
  };
in
if settings == null then homeShell else {
  programs.zsh.enable = true;
  users.users.${if isLive then "nixos" else settings.account.name}.shell = pkgs.zsh;
  home-manager.users = lib.mkIf (!isLive) {
    ${settings.account.name} = homeShell;
  };
}
